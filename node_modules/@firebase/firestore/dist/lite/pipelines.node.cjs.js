...
class OptionsUtil {
    ...
    _getKnownOptions(options, context) {
        const knownOptions = common126a6318_node.ObjectValue.empty();
        ...
        for (const knownOptionKey in this.optionDefinitions) {
            if (this.optionDefinitions.hasOwnProperty(knownOptionKey)) {
                ...
                if (knownOptionKey in options) {
                    ...
                    if (optionDefinition.nestedOptions && common126a6318_node.isPlainObject(optionValue)) {
                        ...
                        protoValue = common126a6318_node.parseData(optionValue, context) ?? undefined;
                    }
                    if (protoValue) {
                        knownOptions.set(common126a6318_node.FieldPath$1.fromServerFormat(optionDefinition.serverName), protoValue);
                    }
                }
            }
        }
    ...
    getOptionsProto(context, knownOptions, optionsOverride) {
        ...
        if (optionsOverride) {
            const optionsMap = new Map(common126a6318_node.mapToArray(optionsOverride, (value, key) => [
                common126a6318_node.FieldPath$1.fromServerFormat(key),
                value !== undefined ? common126a6318_node.parseData(value, context) : null
            ]));
...
function isIMapValue(obj) {
    ...
    if ('fields' in obj && (obj.fields === null || common126a6318_node.isPlainObject(obj.fields))) {
        return true;
    }
    return false;
}
...
function valueToDefaultExpr$1(value) {
    ...
    else if (common126a6318_node.isPlainObject(value)) {
        result = _map(value);
    }
...
function vectorToExpr$1(value) {
    ...
    else if (value instanceof common126a6318_node.VectorValue) {
        return constant(value);
    }
    else if (Array.isArray(value)) {
        return constant(common126a6318_node.vector(value));
    }
    else {
...
function fieldOrExpression$1(value) {
    if (common126a6318_node.isString(value)) {
...
class Expression {
    ...
    asBoolean() {
        if (this instanceof BooleanExpression) {
            ...
            throw new common126a6318_node.FirestoreError('invalid-argument', `Conversion of type ${typeof this} to BooleanExpression not supported.`);
        }
    }
    subtract(subtrahend) {
    ...
    timestampTruncate(granularity, timezone) {
        const internalGranularity = common126a6318_node.isString(granularity)
...
function _field(nameOrPath, methodName) {
    if (typeof nameOrPath === 'string') {
        if (common126a6318_node.DOCUMENT_KEY_NAME === nameOrPath) {
            return new Field(common126a6318_node.documentId()._internalPath, methodName);
        }
        return new Field(common126a6318_node.fieldPathFromArgument('field', nameOrPath), methodName);
    }
    else {
...
class Constant extends Expression {
     ...
     */
    _toProto(_) {
        common126a6318_node.hardAssert(this._protoValue !== undefined, 0x00ed);
    ...
    _readUserData(context) {
        ...
        if (isFirestoreValue(this._protoValue)) {
            ...
            this._protoValue = common126a6318_node.parseData(this.value, context);
        }
    }
}
...
class MapValue extends Expression {
    ...
    _toProto(serializer) {
        return common126a6318_node.toMapValue(serializer, this.plainObject);
    }
}
/**
 * @beta
...
class Ordering {
    ...
    _toProto(serializer) {
        return {
            mapValue: {
                fields: {
                    direction: common126a6318_node.toStringValue(this.direction),
...
function isSelectable(val) {
    const candidate = val;
    return (candidate.selectable && common126a6318_node.isString(candidate.alias) && isExpr(candidate.expr));
}
...
function isAliasedAggregate(val) {
    const candidate = val;
    return (common126a6318_node.isString(candidate.alias) &&
...
function toPipelineBooleanExpr(f) {
    if (f instanceof common126a6318_node.FieldFilter) {
        ...
        switch (f.op) {
            ...
            default:
                common126a6318_node.fail(0x9047);
        }
    }
    else if (f instanceof common126a6318_node.CompositeFilter) {
        switch (f.op) {
            ...
            default:
                common126a6318_node.fail(0x89ea);
        }
    }
...
function toPipeline(query, db) {
    let pipeline;
    if (common126a6318_node.isCollectionGroupQuery(query)) {
        pipeline = db.pipeline().collectionGroup(query.collectionGroup);
    }
    else if (common126a6318_node.isDocumentQuery(query)) {
        pipeline = db.pipeline().documents([common126a6318_node.doc(db, query.path.canonicalString())]);
    }
    else {
    ...
    // orders
    const orders = common126a6318_node.queryNormalizedOrderBy(query);
...
class AddFields extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...
            args: [common126a6318_node.toMapValue(serializer, this.fields)]
        };
    }
    _readUserData(context) {
...
class Aggregate extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [
                common126a6318_node.toMapValue(serializer, this.accumulators),
                common126a6318_node.toMapValue(serializer, this.groups)
            ]
        };
    }
    _readUserData(context) {
...
class Distinct extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common126a6318_node.toMapValue(serializer, this.groups)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
    ...
    _toProto(serializer) {
        return {
            ...
            args: [
                ...
                common126a6318_node.toStringValue(this.distanceMeasure)
            ]
        };
    }
    _readUserData(context) {
...
class Limit extends Stage {
    ...
    constructor(limit, options) {
        common126a6318_node.hardAssert(!isNaN(limit) && limit !== Infinity && limit !== -Infinity, 0x882c);
        super(options);
        this.limit = limit;
    }
    /**
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common126a6318_node.toNumber(serializer, this.limit)]
        };
    }
}
/**
...
class Offset extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common126a6318_node.toNumber(serializer, this.offset)]
        };
    }
}
/**
...
class Select extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common126a6318_node.toMapValue(serializer, this.selections)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Sample extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common126a6318_node.toNumber(serializer, this.rate), common126a6318_node.toStringValue(this.mode)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Union extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common126a6318_node.toPipelineValue(this.other._toProto(serializer))]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Replace extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [this.map._toProto(serializer), common126a6318_node.toStringValue(Replace.MODE)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
function readUserDataHelper(expressionMap, context) {
    if (common126a6318_node.isUserData(expressionMap)) {
...
class PipelineSource {
    ...
    collection(collectionOrOptions) {
        ...
        const options = common126a6318_node.isString(collectionOrOptions) ||
            common126a6318_node.isCollectionReference(collectionOrOptions)
            ? {}
            : collectionOrOptions;
        const collectionRefOrString = common126a6318_node.isString(collectionOrOptions) ||
            common126a6318_node.isCollectionReference(collectionOrOptions)
            ? collectionOrOptions
            : collectionOrOptions.collection;
        // Validate that a user provided reference is for the same Firestore DB
        if (common126a6318_node.isCollectionReference(collectionRefOrString)) {
            this._validateReference(collectionRefOrString);
        }
        // Convert user land convenience types to internal types
        const normalizedCollection = common126a6318_node.isString(collectionRefOrString)
    ...
    collectionGroup(collectionIdOrOptions) {
        ...
        let collectionId;
        let options;
        if (common126a6318_node.isString(collectionIdOrOptions)) {
    ...
    documents(docsOrOptions) {
        ...
        docs
            .filter(v => v instanceof common126a6318_node.DocumentReference)
            .forEach(dr => this._validateReference(dr));
        // Convert user land convenience types to internal types
        const normalizedDocs = docs.map(doc => common126a6318_node.isString(doc) ? doc : doc.path);
        // Create stage object
    ...
    _validateReference(reference) {
        ...
        if (!refDbId.isEqual(this.databaseId)) {
            throw new common126a6318_node.FirestoreError(common126a6318_node.Code.INVALID_ARGUMENT, `Invalid ${reference instanceof common126a6318_node.CollectionReference
                ? 'CollectionReference'
                : 'DocumentReference'}. ` +
...
class PipelineResult {
    ...
    get(fieldPath) {
        ...
        const value = this._fields.field(common126a6318_node.fieldPathFromArgument('DocumentSnapshot.get', fieldPath));
        if (value !== null) {
...
function selectablesToMap(selectables) {
    ...
    for (const selectable of selectables) {
        ...
        if (typeof selectable === 'string') {
            ...
            common126a6318_node.fail(0x5319, { selectable });
        }
        if (result.get(alias) !== undefined) {
            throw new common126a6318_node.FirestoreError('invalid-argument', `Duplicate alias or field '${alias}'`);
        }
...
function aliasedAggregateToMap(aliasedAggregatees) {
    return aliasedAggregatees.reduce((map, selectable) => {
        if (map.get(selectable.alias) !== undefined) {
            throw new common126a6318_node.FirestoreError('invalid-argument', `Duplicate alias or field '${selectable.alias}'`);
        }
...
function vectorToExpr(value) {
    if (value instanceof Expression) {
        ...
        const result = constant(common126a6318_node.vector(value));
        return result;
    }
    else {
        throw new Error('Unsupported value: ' + typeof value);
...
class Pipeline {
    ...
    removeFields(fieldValueOrOptions, ...additionalFields) {
        // Process argument union(s) from method overloads
        const options = isField(fieldValueOrOptions) || common126a6318_node.isString(fieldValueOrOptions)
            ? {}
            : fieldValueOrOptions;
        const fields = isField(fieldValueOrOptions) || common126a6318_node.isString(fieldValueOrOptions)
            ? [fieldValueOrOptions, ...additionalFields]
            : fieldValueOrOptions.fields;
        // Convert user land convenience types to internal types
        const convertedFields = fields.map(f => common126a6318_node.isString(f) ? field(f) : f);
        // Create stage object
    ...
    select(selectionOrOptions, ...additionalSelections) {
        // Process argument union(s) from method overloads
        const options = isSelectable(selectionOrOptions) || common126a6318_node.isString(selectionOrOptions)
            ? {}
            : selectionOrOptions;
        const selections = isSelectable(selectionOrOptions) || common126a6318_node.isString(selectionOrOptions)
    ...
    offset(offsetOrOptions) {
        ...
        let options;
        let offset;
        if (common126a6318_node.isNumber(offsetOrOptions)) {
            options = {};
            offset = offsetOrOptions;
        }
        else {
    ...
    limit(limitOrOptions) {
        // Process argument union(s) from method overloads
        const options = common126a6318_node.isNumber(limitOrOptions) ? {} : limitOrOptions;
        const limit = common126a6318_node.isNumber(limitOrOptions)
            ? limitOrOptions
    ...
    distinct(groupOrOptions, ...additionalGroups) {
        // Process argument union(s) from method overloads
        const options = common126a6318_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
            ? {}
            : groupOrOptions;
        const groups = common126a6318_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
    ...
    replaceWith(valueOrOptions) {
        // Process argument union(s) from method overloads
        const options = common126a6318_node.isString(valueOrOptions) || isExpr(valueOrOptions) ? {} : valueOrOptions;
        const fieldNameOrExpr = common126a6318_node.isString(valueOrOptions) || isExpr(valueOrOptions)
            ? valueOrOptions
            : valueOrOptions.map;
        // Convert user land convenience types to internal types
    ...
    sample(documentsOrOptions) {
        // Process argument union(s) from method overloads
        const options = common126a6318_node.isNumber(documentsOrOptions) ? {} : documentsOrOptions;
        let rate;
        let mode;
        if (common126a6318_node.isNumber(documentsOrOptions)) {
            rate = documentsOrOptions;
            mode = 'documents';
        }
        else if (common126a6318_node.isNumber(documentsOrOptions.documents)) {
    ...
    unnest(selectableOrOptions, indexField) {
        ...
        if (common126a6318_node.isString(indexFieldName)) {
...
function execute(pipeline) {
    const datastore = common126a6318_node.getDatastore(pipeline._db);
    const udr = new common126a6318_node.UserDataReader(pipeline._db._databaseId, 
    ...
    return common126a6318_node.invokeExecutePipeline(datastore, structuredPipeline).then(result => {
        ...
        const docs = result
            ...
            ? new common126a6318_node.DocumentReference(pipeline._db, null, element.key)
 ...
 * ```
 */
common126a6318_node.Firestore.prototype.pipeline = function () {
    const userDataWriter = new common126a6318_node.LiteUserDataWriter(this);
    const userDataReader = common126a6318_node.newUserDataReader(this);
